...
class gtsAPIManager_mysql extends xPDOGenerator_mysql
{
    ...
    {
        $sql = "ALTER TABLE {$this->manager->xpdo->getTableName($className)} CHANGE `{$data['name']}` " . $this->getColumnDef($data);
    ...
    protected function getColumnDef($data = array())
    {
        ...
        $null = ($data['dbnull'] == 'true' or $data['dbnull'] == 1 or $data['dbnull'] === true) ? ' NULL' : ' NOT NULL';
        $comment = '';
    ...
    {
        $sql = 'SHOW FULL COLUMNS FROM ' . $this->manager->xpdo->config['dbname'] . '.' . $this->manager->xpdo->escape($this->manager->xpdo->getTableName($table));
        $out = array();
        if ($fieldsStmt = $this->manager->xpdo->query($sql)) {
            $fields = $fieldsStmt->fetchAll(PDO::FETCH_ASSOC);
            if (!empty($fields)) {
