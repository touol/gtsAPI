...
class fileGalleryAPIController{
    ...
    public function checkPermissions($rule_action)
    {
        if ($rule_action['authenticated']) {
            if (!$this->modx->user->isAuthenticated()) return $this->error("Не авторизован!");
        }
        if ($rule_action['groups']) {
            $groups = array_map('trim', explode(',', $rule_action['groups']));
            if (!$this->modx->user->isMember($groups)) return $this->error("Нет прав доступа к группе!");
        }
        if ($rule_action['permitions']) {
            $permitions = array_map('trim', explode(',', $rule_action['permitions']));
            foreach ($permitions as $pm) {
                if (!$this->modx->hasPermission($pm)) return $this->error("Нет прав доступа!");
            }
        }
        return $this->success();
    }
