...
trait TableCrudTrait
{
    ...
    public function copy($rule, $request, $action)
    {
        ...
        if (!empty($request['ids'])) {
            ...
            $count = $this->modx->getCount($rule['class'], ['id:IN' => $request['ids']]);
            if ($count == 0) {
                return $this->error('Записи для копирования не найдены');
            }
            
            $old_rows = $this->modx->getIterator($rule['class'], ['id:IN' => $request['ids']]);
        } else {
        ...
        foreach ($old_rows as $old_obj) {
            ...
            if (!$new_obj = $this->modx->newObject($rule['class'], $new_row)) {
                $saved[] = $this->error('Ошибка создания объекта', ['class' => $rule['class']]);
                continue;
            }
            ...
            if ($new_obj->save()) {
                ...
                $resp = $this->success('Объект скопирован', ['id' => $new_obj->get('id'), 'class' => $rule['class']]);
                $saved[] = $resp;
                
                // Копируем связанные таблицы many
                if (!empty($copy_config['child']['many'])) {
                    foreach ($copy_config['child']['many'] as $child_class => $child_alias) {
                        $resp1 = $this->copy_many($rule['class'], $child_class, $child_alias, $old_row, $new_row);
                        $resp1['subtables'] = 1;
                ...
                if (!empty($copy_config['child']['one'])) {
                    foreach ($copy_config['child']['one'] as $child_class => $child_alias) {
                        $resp1 = $this->copy_one($rule['class'], $child_class, $child_alias, $old_row, $new_row);
                        $resp1['subtables'] = 1;
                        $resp1['subtable_name'] = $child_class;
                        $saved[] = $resp1;
                    }
                ...
                $saved[] = $this->error('Ошибка сохранения объекта', ['class' => $rule['class']]);
            }
        }
    ...
    // writeLog вынесен в общий TableLogTrait (переиспользуется деревом).
